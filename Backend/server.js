...
async function startServer() {
  try {
    ...
    const io = new Server.Server(listen, {
      cors: {
        origin: "*",
        methods: ["GET", "POST"],
      },
    ...
    io.on("connection", async (socket) => {
      ...
      console.log(`User connected ${socket.id} and user_id=${user_id}`);

      if (user_id != null && Boolean(user_id)) {
        try {
          const testRes = await User.findByIdAndUpdate(user_id, {
            socket_id: socket.id,
            status: "Online",
          });
        } catch (e) {
      ...
      socket.on("get_direct_conversations", async ({ user_id }, callback) => {
        try {
          ...
          if (existing_conversations.length === 0) {
            callback([]);
            // callback({ message: "No conversations found." });
          } else {
            callback(existing_conversations);
          }
        } catch (error) {
          console.error("Error fetching direct conversations:", error);
          // callback({ message: "An error occurred while fetching conversations." });
          callback([]);
        }
      });
    ...
    if (connectionTrialCount < 5) {
      connectionTrialCount += 1;
      console.log(`Trying to connect to the database ${connectionTrialCount} time!`);
      startServer()
    } else {
